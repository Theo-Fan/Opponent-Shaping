...
from coin_game import CoinGame, episode_stats, coin_game_params, make_zero_episode, eval_agent_against_always_cooperate, eval_agent_against_always_defect
...
WINDOW_EPISODE_METRIC_KEYS = [
    'mean_reward_player0',
    'mean_reward_player1',
    'avg_episode_return_player0',
    'avg_episode_return_player1',
    'avg_pickups_player0',
    'avg_pickups_player1',
...
EPISODE_STATS_KEYS = [
    ...
    'easymisses',
    'adversity',
    'adversarial_pickup_div_timesteps',
    'any_pickup_div_timesteps',
    'adversarial_pickup_div_all_pickup',
    'own_pickup_div_timesteps',
    'nearpasses',
]
EPISODE_STATS_FIELDNAMES = [
    f'{key}_{player}' for key in EPISODE_STATS_KEYS for player in (0, 1)
]
...
def get_num_training_episodes_per_iteration(hp):
    return int(hp['batch_size']) * (1 if hp['just_self_play'] else 2)


def get_num_training_timesteps_per_iteration(hp):
    return get_num_training_episodes_per_iteration(hp) * int(hp['game']['game_length'])


def get_configured_max_train_timestep(hp):
    for key in MAX_TRAIN_TIMESTEP_KEYS:
        if key in hp:
            max_train_timestep = int(hp[key])
            if max_train_timestep <= 0:
                raise ValueError(f'{key} must be positive, got {max_train_timestep}')
            return max_train_timestep
    return None


def get_num_training_iterations(hp):
    max_train_timestep = get_configured_max_train_timestep(hp)
    if max_train_timestep is not None:
        timesteps_per_iteration = get_num_training_timesteps_per_iteration(hp)
        if max_train_timestep % timesteps_per_iteration != 0:
            raise ValueError(
                f'max_train_timestep={max_train_timestep} must be divisible by '
                f'timesteps_per_iteration={timesteps_per_iteration} '
                f'(batch_size={hp["batch_size"]}, just_self_play={hp["just_self_play"]}, '
                f'game_length={hp["game"]["game_length"]})'
            )
        return max_train_timestep // timesteps_per_iteration

    if 'train_num_iterations' in hp:
        num_training_iterations = int(hp['train_num_iterations'])
        if num_training_iterations <= 0:
            raise ValueError(f'train_num_iterations must be positive, got {num_training_iterations}')
        return num_training_iterations

    if 'train_num_episodes' not in hp:
        return DEFAULT_NUM_TRAINING_ITERATIONS

    total_training_episodes = int(hp['train_num_episodes'])
    episodes_per_iteration = get_num_training_episodes_per_iteration(hp)
    if total_training_episodes <= 0:
        raise ValueError(f'train_num_episodes must be positive, got {total_training_episodes}')
    if total_training_episodes % episodes_per_iteration != 0:
        raise ValueError(
            f'train_num_episodes={total_training_episodes} must be divisible by '
            f'episodes_per_iteration={episodes_per_iteration} '
            f'(batch_size={hp["batch_size"]}, just_self_play={hp["just_self_play"]})'
        )
    return total_training_episodes // episodes_per_iteration


def get_total_training_episodes(hp):
    return get_num_training_iterations(hp) * get_num_training_episodes_per_iteration(hp)


def get_total_training_timesteps(hp):
    return get_num_training_iterations(hp) * get_num_training_timesteps_per_iteration(hp)


def get_metrics_log_timestep_freq(hp):
    if 'metrics_log_timestep_freq' in hp:
        freq = int(hp['metrics_log_timestep_freq'])
    else:
        freq = DEFAULT_METRICS_LOG_TIMESTEP_FREQ
    if freq <= 0:
        raise ValueError(f'metrics_log_timestep_freq must be positive, got {freq}')
    return freq
...
def flatten_player_stats(statistics):
    flattened = {}
    for key, values in statistics.items():
        array = np.asarray(jax.device_get(values)).reshape(-1)
        for player, value in enumerate(array):
            flattened[f'{key}_{player}'] = float(value)
    return flattened
...
def append_iteration_metric_row(csv_path, row):
    with open(csv_path, 'a', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=TRAIN_ITERATION_METRICS_FIELDNAMES)
        writer.writerow(row)


def extract_episode_metric_arrays(episodes):
    rewards = np.asarray(jax.device_get(episodes['rew']))
    coin_owner = np.asarray(jax.device_get(episodes['coin_owner']))[..., 0]
    coin_pos = np.asarray(jax.device_get(episodes['coin_pos']))
    player0_pos = np.asarray(jax.device_get(episodes['player1_pos']))
    player1_pos = np.asarray(jax.device_get(episodes['player2_pos']))

    batch_size, trace_length, _ = rewards.shape
    previous_coin_pos = coin_pos[:, :-1]
    coin_owner_per_timestep = coin_owner[:, :-1]

    player0_takes = np.all(player0_pos[:, 1:] == previous_coin_pos, axis=-1)
    player1_takes = np.all(player1_pos[:, 1:] == previous_coin_pos, axis=-1)

    player0_same_color = player0_takes & (coin_owner_per_timestep == 0)
    player0_different_color = player0_takes & (coin_owner_per_timestep == 1)
    player1_same_color = player1_takes & (coin_owner_per_timestep == 1)
    player1_different_color = player1_takes & (coin_owner_per_timestep == 0)

    return {
        'mean_reward_player0': rewards[:, :, 0].mean(axis=1),
        'mean_reward_player1': rewards[:, :, 1].mean(axis=1),
        'avg_episode_return_player0': rewards[:, :, 0].sum(axis=1),
        'avg_episode_return_player1': rewards[:, :, 1].sum(axis=1),
        'avg_pickups_player0': player0_takes.sum(axis=1),
        'avg_pickups_player1': player1_takes.sum(axis=1),
        'avg_same_color_pickups_player0': player0_same_color.sum(axis=1),
        'avg_same_color_pickups_player1': player1_same_color.sum(axis=1),
        'avg_different_color_pickups_player0': player0_different_color.sum(axis=1),
        'avg_different_color_pickups_player1': player1_different_color.sum(axis=1),
        'num_episodes': batch_size,
        'game_length': trace_length,
    }


def compute_iteration_metric_row(
        episode_batches,
        iteration,
        episode,
        timestep,
    ...
    extracted_batches = [extract_episode_metric_arrays(episodes) for episodes in episode_batches]
    metric_keys = [
        'mean_reward_player0',
        'mean_reward_player1',
        'avg_episode_return_player0',
    ...
    num_episodes_in_window = sum(batch['num_episodes'] for batch in extracted_batches)
    game_length = extracted_batches[0]['game_length']
    num_timesteps_in_window = num_episodes_in_window * game_length
    ...
    row = {
        ...
        'num_episodes_in_window': num_episodes_in_window,
        'num_timesteps_in_window': num_timesteps_in_window,
        'game_length': game_length,
    }

    for key in metric_keys:
        values = np.concatenate([batch[key] for batch in extracted_batches], axis=0)
        row[key] = float(values.mean())

    for player in (0, 1):
        same = row[f'avg_same_color_pickups_player{player}']
        different = row[f'avg_different_color_pickups_player{player}']
        row[f'same_color_pickup_ratio_player{player}'] = float(same / max(same + different, 1e-8))
    ...
    if statistics is not None:
        row.update(flatten_player_stats(statistics))
    return row
...
def train(hp, log_wandb):
    ...
    if metrics_log_timestep_freq % timesteps_per_iteration != 0:
        print(
            ...
            'iteration that reaches or passes the requested timestep frequency.'
        )
    ...
    for i in range(num_training_iterations):
        state['step'] = completed_timesteps
        ...
        state['step'] = completed_timesteps
        update_metric_row = {
            'loss_agent_0_qvalue': scalar_mean(loss_agent_0_qvalue),
            'agent0_entropy': scalar_mean(agent0_entropy),
            'grad_agent0_norm': scalar_mean(grad_agent0_norm),
            'grad_opponent0_norm': scalar_mean(grad_opponent0_norm),
            'walltime': time.time() - start_time,
        }
        if not just_self_play:
            update_metric_row.update({
                'loss_agent_1_qvalue': scalar_mean(loss_agent_1_qvalue),
                'agent1_entropy': scalar_mean(agent1_entropy),
        ...
        if i % hp['eval_every'] == 0:
            ...
            episodes_eval = generate_eval_episodes(agent0=state['agent0'], agent1=state['agent1'], rng=rng, carries=carries, hp=hp)
...
@partial(jax.jit, static_argnames=('hp',))
def generate_eval_episodes(agent0, agent1, carries, rng, hp):
...
def play_episode_scan_inner_gru(inp, trace_length, env):
    ...
    def body_fn(carry, _):
        ...
        act = jp.stack([action1, action2])
        logp = jp.stack([logp1, logp2], axis=0)
        assert act.shape == (2,)
        ...
        return (env, rng, episode, t + 1, c_0_actor, c_0_qvalue, c_1_actor, c_1_qvalue), aux
